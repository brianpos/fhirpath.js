...
import Terminologies from './terminologies'; // TypeScript module
import { ResourceNode, TypeInfo } from './types';
import { FHIRPathContext } from './types/core';
import util from './utilities';
const urlJoin = require('@loxjs/url-join'); // External JS module

interface AdditionalEngine {
  [key: string]: any;
  memberOf: (coll: any[], valueSetColl: any[]) => Promise<boolean> | any[];
  resolveFn: (coll: any[]) => Promise<any[]>;
}

let engine: AdditionalEngine = {} as AdditionalEngine;
 ...
 *  specified value set.
 */
engine.memberOf = function (this: FHIRPathContext, coll: any[], valueSetColl: any[]): Promise<boolean> | any[] {
  const ctx = this;
  ...
  if (coll.length === 1 && coll[0] != null && valueSetColl.length === 1 && valueSetColl[0] != null) {
    ...
    if (typeInfo.is(TypeInfo.FhirUri, ctx.model) || typeInfo.is(TypeInfo.SystemString)) {
      ...
      const result = Terminologies.validateVS.call(this,
        [terminologies], valueSetColl, coll, ''
      );
      return result?.then((params: any) => {
        return util.valData(params)?.parameter.find((p: any) => p.name === "result").valueBoolean;
      }, () => []) || [];
    }
  }
 ...
 */
function requestResourceByCanonicalUrl(ctx: FHIRPathContext, refType: string, url: string): Promise<any | null> {
  const fhirServerUrl = ctx.processedVars.fhirServerUrl;
  if (!fhirServerUrl) {
    throw new Error('Option "fhirServerUrl" is not specified.');
  }
  const match = /^(https?:\/\/[^|]*)(\|(.*))?/.exec(url);
  if (refType && match && ctx.model && ctx.model.resourcesWithUrlParam[refType]) {
    const params: { [key: string]: string } = { url: match[1] };
    if (match[3]) {
 ...
 */
function getContainedResource(resource: any, containedResId: string): any | null {
  return containedResId ?
    util.valData(resource)?.contained?.find((r: any) => r.id === containedResId) || null
    : null;
}
 ...
 * @type {Object}
 */
const baseResourceTypes: { [key: string]: number } = { Resource: 1, DomainResource: 1 };
 ...
 *  if found, or null.
 */
function requestResourceByUrl(ctx: FHIRPathContext, node: any, refType: string | null | undefined, url: string, isCanonical: boolean): Promise<any | null> {
  let promiseOfResource: Promise<any> | null = null;
  let fragment = '';
  ...
  if (promiseOfResource) {
    return promiseOfResource.then((res: any) => {
      if (fragment) {
 ...
 *  objects.
 */
engine.resolveFn = function (this: FHIRPathContext, coll: any[]): Promise<any[]> {
  const ctx = this;

  util.checkAllowAsync(ctx, 'resolve');

  return Promise.allSettled((coll || []).reduce((acc: Promise<any>[], item: any) => {
    ...
    acc.push(res as Promise<any>);
    return acc;
  }, [])).then((result: any[]) => {
    return result.reduce((acc: any[], resItem: any) => {
