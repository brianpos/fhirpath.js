...
import additional from "./additional"; // TypeScript
...
import misc from "./misc"; // TypeScript
import navigation from "./navigation";
import * as parser from './parser';
import { ASTNode } from './parser';
import './polyfill';
import supplements from "./sdc-ig-supplements"; // TypeScript
...
import {
  ...
  FP_Quantity,
  FP_Time,
  FP_Type, Model, ResourceNode, TypeInfo
} from './types';
...
interface InvocationTableEntry {
  fn: Function;
  arity?: { [key: number]: string[] };
  nullable?: boolean;
}

interface FhirPathEngine {
  // [key: string]: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  evalTable: { [key: string]: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any };
  invocationTable: { [key: string]: InvocationTableEntry };
  doEval: (ctx: FHIRPathContext, data: any[], node: ASTNode) => any;
  doEvalSync: (ctx: FHIRPathContext, data: any[], node: ASTNode) => any;
  InvocationExpression: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  TermExpression: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  PolarityExpression: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  TypeSpecifier: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  ExternalConstantTerm: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  LiteralTerm: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  StringLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  BooleanLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  QuantityLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  DateLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  DateTimeLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  TimeLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  NumberLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  LongNumberLiteral: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  Identifier: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  InvocationTerm: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  MemberInvocation: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  IndexerExpression: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  Functn: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  realizeParams: (ctx: FHIRPathContext, parentData: any[], args: ASTNode[]) => any;
  FunctionInvocation: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  ParamList: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  UnionExpression: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  ThisInvocation: (ctx: FHIRPathContext) => any;
  TotalInvocation: (ctx: FHIRPathContext) => any;
  IndexInvocation: (ctx: FHIRPathContext) => any;
  OpExpression: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  NullLiteral: () => [];
  AliasOpExpression: (map: Record<string, string>) => (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
  ParenthesizedTerm: (ctx: FHIRPathContext, parentData: any[], node: ASTNode) => any;
}

let engine: FhirPathEngine = {} as FhirPathEngine; // the object with all FHIRPath functions and operations
let makeResNode = ResourceNode.makeResNode;
// Terminologies and Factory are not used in this file, so we can comment them out
...
// let doEval: (ctx: FHIRPathContext, data: any[], node: ASTNode) => any;


engine.InvocationExpression = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return node.children?.reduce((acc: any, ch: ASTNode) => {
    return engine.doEval(ctx, acc, ch);
  }, parentData);
};

engine.TermExpression = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  if (parentData) {
...
engine.PolarityExpression = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.TypeSpecifier = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  let namespace, name;
...
engine.ExternalConstantTerm = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  let varName;
  ...
  if (varName in ctx.vars && !ctx.processedUserVarNames?.has(varName)) {
    ...
    if (!ctx.processedUserVarNames) {
      ctx.processedUserVarNames = new Set();
    }
...
engine.LiteralTerm = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.StringLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any[] {
  return [getStringLiteralVal(node.text)];
};

function getStringLiteralVal(str: string): string {
...
engine.BooleanLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  if (node.text === "true") {
    return [true];
  } else {
    return [false];
...
engine.QuantityLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.DateLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  var dateStr = node.text.slice(1); // Remove the @
  return [new FP_Date(dateStr)];
};

engine.DateTimeLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  var dateTimeStr = node.text.slice(1); // Remove the @
  return [new FP_DateTime(dateTimeStr)];
};

engine.TimeLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  var timeStr = node.text.slice(1); // Remove the @
  return [new FP_Time(timeStr)];
};

engine.NumberLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return [Number(node.text)];
};

engine.LongNumberLiteral = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return [BigInt(node.text.slice(0, -1))];
};

engine.Identifier = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return [getIdentifierVal(node.text)];
};

function getIdentifierVal(str: string): string {
  return str.replace(/`([^`]*)`/g, '$1');
}

engine.InvocationTerm = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return engine.doEval(ctx, parentData, node.children![0]);
};

engine.MemberInvocation = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.IndexerExpression = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.Functn = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return node.children!.map(function (x) {
    return engine.doEval(ctx, parentData, x);
  });
};

engine.realizeParams = function (ctx: FHIRPathContext, parentData: any[], args: ASTNode[]): any {
...
function makeParam(ctx: FHIRPathContext, parentData: any[], type: string, param: ASTNode): any {
  if (type === "Expr") {
...
function doInvoke(ctx: FHIRPathContext, fnName: string, data: any, rawParams: any[]) {
  var invoc =
...
function isNullable(x: any): boolean {
  return x === null || x === undefined || util.isEmpty(x);
}

function infixInvoke(ctx: FHIRPathContext, fnName: string, data: any, rawParams: any[]): any {
  ...
  if (invoc && invoc.fn) {
    ...
    var argTypes = invoc.arity ? invoc.arity[paramsNumber] : undefined;
    if (argTypes) {
      var params = [];
      for (var i = 0; i < paramsNumber; i++) {
        var tp = argTypes[i];
...
engine.FunctionInvocation = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.ParamList = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
...
engine.UnionExpression = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return infixInvoke(ctx, '|', parentData, node.children!);
};

engine.ThisInvocation = function (ctx: FHIRPathContext): any {
  return ctx.$this;
};

engine.TotalInvocation = function (ctx: FHIRPathContext): any {
  return util.arraify(ctx.$total);
};

engine.IndexInvocation = function (ctx: FHIRPathContext): any {
  return util.arraify(ctx.$index);
};

engine.OpExpression = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  var op = node.terminalNodeText[0];
  return infixInvoke(ctx, op, parentData, node.children!);
};

engine.AliasOpExpression = function (map: Record<string, string>) {
  return function (ctx: FHIRPathContext, parentData: any[], node: ASTNode) {
    var op = node.terminalNodeText[0];
    var alias = map[op];
...
engine.ParenthesizedTerm = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode): any {
  return engine.doEval(ctx, parentData, node.children![0]);
};
...
const rawTable: Record<string, any> = engine;

engine.doEval = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode) {
...
engine.doEvalSync = function (ctx: FHIRPathContext, parentData: any[], node: ASTNode) {
  const evaluator = engine.evalTable[node.type];
  if (evaluator) {
    return evaluator.call(engine, ctx, parentData, node);
  } else {
    if (rawTable[node.type]) {
      return rawTable[node.type].call(engine, ctx, parentData, node);
    }
 ...
 */
function applyParsedPath(resource: any, parsedPath: any, envVars?: Record<string, any>, model?: Model, options?: any): any {
  constants.reset();
 ...
 * @return {Array}
 */
function prepareEvalResult(result: any, model?: Model, options?: any): any {
  return result
 ...
 */
function resolveInternalTypes(val: any): any {
 ...
 *   to abort the asynchronous FHIRPath expression evaluation.
 */
function evaluate(fhirData: any, path: string, envVars?: Record<string, any>, model?: Model, options?: any): any {
  var compiledExpression = compile(path, model, options);
  return compiledExpression(fhirData, envVars);
}
 ...
 *   problems.
 */
function compile(path: string | { base: string, expression: string }, model?: Model, options?: any): (fhirData: any, envVars?: Record<string, any>, additionalOptions?: any) => any {
  options = {
    resolveInternalTypes: true,
    ...options
  };

  // Tweak the userInvocationTable to rework the function to de-compose the ResourceNode when required
  const userInvocationTable: Record<string, { fn: Function, arity: Record<number, string[]>, internalStructures?: boolean }> = options.userInvocationTable;
  if (userInvocationTable) {
    options.userInvocationTable = Object.keys(userInvocationTable).reduce(
      ...
      }, {} as Record<string, { fn: Function, arity: Record<number, string[]>, internalStructures?: boolean }>);
  }

  if (typeof path === 'object') {
    const node = parse(path.expression);
    let basePath = path.base?.replace(/\[\d*]/g, '');
    let baseFhirNodeDataType: string | undefined;
    if (basePath) {
      // Normalize basePath by replacing long recursive paths with their base
      // paths, e.g. Questionnaire.item.item -> Questionnaire.item
      const arrayOfPathsDefinedElsewhere = model && Object.keys(model.pathsDefinedElsewhere);
      let changed;
      do {
    ...
    return function (fhirData: any, envVars?: Record<string, any>, additionalOptions?: any) {
      if (basePath) {
...
/**
 * Returns the type of each element in fhirpathResult array.
 */
function typesFn(fhirpathResult: any): string[] {
  return util.arraify(fhirpathResult).map((value: any) => {
...
// Named exports to match the .d.ts file
export { compile, evaluate, parse, resolveInternalTypes, typesFn as types, version };
