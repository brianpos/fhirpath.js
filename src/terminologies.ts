...
export interface TerminologyInvocationEntry {
  fn: Function;
  arity: Record<number, string[]>;
}

export interface TerminologyInvocationTable {
  [key: string]: TerminologyInvocationEntry;
}
...
export class Terminologies {
  ...
  invocationTable: TerminologyInvocationTable;

  constructor(terminologyUrl: string) {
  ...
  static invocationTable: TerminologyInvocationTable = {
    expand: {
      fn: Terminologies.expand, arity: {
        1: ['AnyAtRoot'],
        2: ['AnyAtRoot', 'AnySingletonAtRoot']
      }
    },
    lookup: {
      fn: Terminologies.lookup, arity: {
        1: ['AnyAtRoot'],
        2: ['AnyAtRoot', 'AnySingletonAtRoot']
      }
    },
    validateVS: {
      fn: Terminologies.validateVS, arity: {
        2: ['AnyAtRoot', 'AnyAtRoot'],
        3: ['AnyAtRoot', 'AnyAtRoot', 'AnySingletonAtRoot']
      }
    },
    validateCS: {
      fn: Terminologies.validateCS, arity: {
        2: ['AnyAtRoot', 'AnyAtRoot'],
        3: ['AnyAtRoot', 'AnyAtRoot', 'AnySingletonAtRoot']
      }
    },
    subsumes: {
      fn: Terminologies.subsumes, arity: {
        3: ['AnyAtRoot', 'AnyAtRoot', 'AnyAtRoot'],
        4: ['AnyAtRoot', 'AnyAtRoot', 'AnyAtRoot', 'AnySingletonAtRoot']
      }
    },
    translate: {
      fn: Terminologies.translate, arity: {
        2: ['AnyAtRoot', 'AnyAtRoot'],
        3: ['AnyAtRoot', 'AnyAtRoot', 'AnySingletonAtRoot']
      }
    }
   ...
   */
  static expand(this: FHIRPathContext, self: any, valueSetColl: ResourceNode[] | string[], params: string = '') {
    let response = null;
    const ctx = this;
   ...
   */
  static lookup(this: FHIRPathContext, self: any, codedColl: ResourceNode[] | string[], params: string = '') {
    let response = null;
    const ctx = this;
    ...
    if (codedColl.length === 1 && checkParams(params)) {
      ...
      if (isCodeableConcept || isCoding || isCode) {
        ...
        response = util.fetchWithCache(`${self[0].terminologyUrl}/CodeSystem/$lookup`, {
          method: "POST",
   ...
   *  the validation operation.
   */
  static validateVS(this: FHIRPathContext, self: any, valueSetColl: ResourceNode[] | string[], codedColl: ResourceNode[] | string[], params: string = '') {
    let response = null;
    const ctx = this;
      ...
      if (isActualValueSet || isValueSetUrl) {
        ...
        if (isCodeableConcept || isCoding || isCode) {
          ...
          if (isActualValueSet || isCodeableConcept && coded.coding?.length !== 1) {
            ...
            response = (isCode ? getSystemFromVS(ctx, self[0].terminologyUrl, valueSet) : Promise.resolve()).then(system => {
              const codedParamName = isCodeableConcept ? 'codeableConcept' : isCoding ? 'coding' : 'code';
              const parameters = {
                resourceType: 'Parameters',
                parameter: [
                  {
              ...
              return util.fetchWithCache(requestUrl, {
                method: "POST",
                body: util.toJSON(parameters),
                ...(ctx.signal ? { signal: ctx.signal } : {})
              });
   ...
   *  the validation operation.
   */
  static validateCS(this: FHIRPathContext, self: any, codeSystemColl: ResourceNode[] | string[], codedColl: ResourceNode[] | string[], params: string = '') {
    let response = null;
    const ctx = this;
    ...
    if (codeSystemColl.length === 1 && codedColl.length === 1 &&
      ...
      if (isActualCodeSystem || isCodeSystemUrl) {
        ...
        if (isCodeableConcept || isCoding || isCode) {
          ...
          response = util.fetchWithCache(requestUrl, {
            method: "POST",
            body: util.toJSON(parameters),
            ...(ctx.signal ? { signal: ctx.signal } : {})
          });
   ...
   */
  static subsumes(this: FHIRPathContext, self: any, systemColl: ResourceNode[] | string[], coded1Coll: ResourceNode[] | string[], coded2Coll: ResourceNode[] | string[], params: string = '') {
    let response = null;
    const ctx = this;
    ...
    return response && response.then(obj => {
      if (obj?.resourceType === 'Parameters') {
        const code = obj.parameter?.find((p: any) => p.name === 'outcome')?.valueCode;
   ...
   */
  static translate(this: FHIRPathContext, self: any, conceptMapColl: ResourceNode[] | string[], codedColl: ResourceNode[] | string[], params: string = '') {
    let response = null;
    const ctx = this;
    ...
    if (conceptMapColl.length === 1 && codedColl.length === 1 &&
      ...
      if (isActualConceptMap || isConceptMapUrl) {
        ...
        if (isCoding || isCode) {
          ...
          const m = modelToTranslateSourceParamName[ctx.model?.version ?? 'r4']; // defaulting to r4 for terminology if no model was provided
          const codedParamName = isCodeableConcept ?
 ...
 */
function getSystemFromVS(ctx: FHIRPathContext, terminologyUrl: any, valueSet: any | string): Promise<string> {
  const queryParams = new URLSearchParams({ url: valueSet });
 ...
 */
function getSystemFromArrayItems(arr: any[], system?: string) {
  if (arr) {
 ...
 *    by `getParamValue`.
 */
function toFhirParameters(params: string): { name: string, [key: string]: any }[] {
  const parsed: { name: string, [key: string]: any }[] = [];
 ...
 */
const modelToTranslateSourceParamName: Record<string, { sourceCoding: string, sourceCode: string, sourceCodeableConcept: string }> = {
  r4: {
 ...
 * @type {Object}
 */
const paramName2valueXName: Record<string, string> = Object.entries({
 ...
 */
function getParamValue(fieldName: string, value: string) {
  let paramValue: Record<string, any> = {};
  switch (fieldName) {
    case 'valueInteger': {
      let v;
 ...
 */
function getCodedType(ctx: FHIRPathContext, codedColl: ResourceNode[] | string[]) {
 ...
 */
function transformResponseToResource(ctx: any, response: Promise<any> | null, resourceType: string): Promise<any> | null {
