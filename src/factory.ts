...
import { ResourceNode, TypeInfo, dateRE, dateTimeRE, instantRE, timeRE } from './types';
...
export interface InvocationTable {
  [key: string]: InvocationEntry;
}

export class Factory {
  // Same as fhirpath.invocationTable, but for %factory methods
  static invocationTable: InvocationTable = {
  ...
  static {
    [
      {
        type: 'string',
        getValue: function (v: any): string {
      ...
      Factory.invocationTable[primitiveType] = {
         ...
         */
        fn: function (self: any, valueColl: any[], extensions: any[]) {
          let data;
          ...
          if (extensions?.length > 0) {
            _data = {
              extension: extensions.map((e: any) => {
   ...
   * @return {ResourceNode}
   */
  static Extension(self: any, url: string, value: any) {
    if (value.length !== 1) {
      if (value.length > 1) {
   ...
   * @return {{[p: string]: *, url}}
   */
  static createExtensionObject(url: string, value: any) {
   ...
   * @return {ResourceNode}
   */
  static Identifier(self: any, system: string, value: string, use: string, typeColl: ResourceNode[]) {
   ...
   * @return {ResourceNode}
   */
  static HumanName(self: any, family: string, givenColl: any[], prefix: string, suffix: string, text: string, use: string) {
    const data: any = {};
    if (util.isSome(family)) {
      data.family = family;
    }
    if (util.isSome(givenColl)) {
      data.given = givenColl.map((given: any) => {
   ...
   * @return {ResourceNode}
   */
  static ContactPoint(self: any, system: string, value: string, use: string) {
    const data: any = {};
    if (util.isSome(system)) {
      data.system = system;
    }
   ...
   * @return {ResourceNode}
   */
  static Address(self: ResourceNode[], lineColl: string[] | ResourceNode[], city: string, state: string, postalCode: string, country: string, use: string, type: string) {
    const data: any = {}; // this is really the FHIR Address Type, but which one? make the choice of all their types?
   ...
   * @return {ResourceNode}
   */
  static Quantity(self: any, system: string, code: string, value: string, unit: string) {
    const data: any = {};
    if (util.isSome(system)) {
      data.system = system;
    }
   ...
   * @return {ResourceNode}
   */
  static Coding(self: any, system: string, code: string, display: string, version: string) {
    const data: any = {};
    if (util.isSome(system)) {
      data.system = system;
    }
   ...
   * @return {ResourceNode}
   */
  static CodeableConcept(self: any, valueColl: ResourceNode[], text: string) {
    const data: any = valueColl?.length > 0 ?
      {
   ...
   * @return {ResourceNode}
   */
  static create(self: any, typeInfo: TypeInfo) {
   ...
   * @return {ResourceNode|[]}
   */
  static withExtension(self: any, instanceColl: ResourceNode[], url: string, value: ResourceNode[]) {
   ...
   * @return {ResourceNode|*[]}
   */
  static withProperty(self: any, instanceColl: ResourceNode[], name: string, value: ResourceNode[]) {
    if (instanceColl.length > 1) {
      throw new Error("Unexpected collection " + util.toJSON(instanceColl) +
