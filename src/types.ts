...
export interface TypeInfoConstructorArgs {
  name: string;
  namespace?: string;
  refType?: any;
}
...
export class FP_Type {
  ...
  equals(_otherObj: any): boolean | undefined | null {
    return false;
  }
  ...
  /**
     ...
     */
  // eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
  equivalentTo(_otherObj: any): boolean | undefined {
    return false;
  }

  toString(): string {
    return this.asStr ? this.asStr : Object.prototype.toString.call(this);
  }
  ...
  /**
     ...
     */
  // eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
  compare(_otherObj: any): number | null {
  ...
  /**
     ...
     */
  // eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
  plus(_otherObj: any): FP_Type | null {
    throw new Error('Addition not implemented for ' + this.constructor.name);
  }
  ...
  /**
     ...
     */
  // eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
  mul(_otherObj: any): FP_Type | null {
    throw new Error('Multiplication not implemented for ' + this.constructor.name);
  }
  ...
  /**
     ...
     */
  // eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
  div(_otherObj: any): FP_Type | null {
    throw new Error('Division not implemented for ' + this.constructor.name);
  }
}
...
export class FP_Quantity extends FP_Type {
  ...
  equals(otherQuantity: FP_Quantity): boolean | null {
  ...
  /**
     ...
     */
  convToUcumUnits(quantity: FP_Quantity, unitInSeconds: number | undefined) {
    if (unitInSeconds) {
      return {
  ...
  /**
     ...
     * @private
     */
  _compareYearsAndMonths(otherQuantity: FP_Quantity) {
  ...
  static getEquivalentUcumUnitCode(unit: string): string {
    return FP_Quantity.mapTimeUnitsToUCUMCode[unit] || unit.replace(/^'|'$/g, '');
  }

  /**
   * Converts FHIR path value/unit to UCUM value/unit. Usable for comparison.
   */
  static toUcumQuantity(value: number, unit: string): UCUMQuantity {
...
FP_Quantity.mapTimeUnitsToUCUMCode = Object.keys(FP_Quantity.mapUCUMCodeToTimeUnits)
  .reduce(function (res: Record<string, string>, key) {
    res[FP_Quantity.mapUCUMCodeToTimeUnits[key]] = key;
    res[FP_Quantity.mapUCUMCodeToTimeUnits[key] + 's'] = key;
    return res;
  }, {});

abstract class FP_TimeBase extends FP_Type {
  static timeUnitToAddFn: Record<string, (date: Date, amount: number) => Date>;

  // Abstract methods for derived classes to implement
  abstract getTimeUnitToDatePrecision(): Record<string, number>;
  abstract getDatePrecisionToTimeUnit(): string[];
  abstract _dateAtPrecision(precision: any): Date;

  precision?: number;
  timeMatchData?: RegExpMatchArray | null;
  dateObj?: Date | null;

  constructor(timeStr: string | undefined) {
    super();
    this.asStr = timeStr;
  }
  ...
  plus(timeQuantity: FP_Quantity) {
    ...
    const unitPrecision = this.getTimeUnitToDatePrecision()[timeUnit];
    if (unitPrecision === undefined) {
      throw new Error('Unsupported unit for +.  The unit should be one of ' +
        Object.keys(this.getTimeUnitToDatePrecision()).join(', ') + '.');
    }
    let qVal = timeQuantity.value;
    const isTime = (this instanceof FP_Time);
    ...
    let thisPrecision = this._getPrecision() ?? 0;
    if (thisPrecision < unitPrecision) {
      const neededUnit = this.getDatePrecisionToTimeUnit()[thisPrecision];
    ...
    return new (this.constructor as any)(newDateStr);
  }
  ...
  equals(otherDateTime: FP_TimeBase): boolean | undefined {
    ...
    if (!(otherDateTime instanceof this.constructor) && !(this instanceof otherDateTime.constructor))
      ...
      var thisPrec = this._getPrecision() ?? 0;
      var otherPrec = otherDateTime._getPrecision() ?? 0;
  ...
  compare(otherTime: FP_TimeBase): number | null {
    var thisPrecision = this._getPrecision() ?? 0;
    var otherPrecision = otherTime._getPrecision() ?? 0;
  ...
  abstract _getMatchData(): RegExpMatchArray | null | undefined;
  abstract _getTimeParts(): string[];
  ...
  /**
     ...
     */
  _getMatchDataWithPrecision(regEx: RegExp, maxPrecision: number) {
  ...
  /**
     ...
     */
  _getTimePartsBase(timeMatchData: string[] | null | undefined) {
    var timeParts: string[] = [];
    if (!timeMatchData) return timeParts;
  ...
  _getDateObj() {
    if (!this.dateObj) {
      var precision = this._getPrecision() ?? 0;
  ...
  /**
     ...
     *  specified timezone.
     */
  _createDate(year: number, month: number, day: number, hour: number, minutes: number, seconds: number, ms: number, timezoneOffset: string) {
...
class FP_DateTime extends FP_TimeBase {
  ...
  /**
     ...
     */
  _getMatchData() {
    return super._getMatchDataWithPrecision(dateTimeRE, 5);
  }
        ...
        if (month) { // Remove other information from year
          ...
          if (day) { // Remove day information from month
            ...
            if (time) { // Remove time from day
              ...
              let tdm = timeMatchData.slice(3);
              this.timeParts = this.timeParts.concat(
                super._getTimePartsBase(tdm));
            }
          }
        }
      }
  ...
  /**
     ...
     */
  _dateAtPrecision(precision: any) {
    var timeParts = this._getTimeParts() ?? [];
    var md = this._getMatchData() ?? [];
    var timezoneOffset = md.length > 7 ? md[7] : '';
  ...
  static checkString(str: string): FP_DateTime | null {
    let d: FP_DateTime | null = new FP_DateTime(str);
    if (!d._getMatchData())
      d = null;
    return d;
  }
   ...
   */
  static _timeUnitToDatePrecision = {
    "year": 0,
    "month": 1,
   ...
   */
  static _datePrecisionToTimeUnit = [
    "year", "month", "day", "hour", "minute", "second", "millisecond"
  ];

  // Implement abstract methods from FP_TimeBase
  getTimeUnitToDatePrecision(): Record<string, number> {
    return FP_DateTime._timeUnitToDatePrecision;
  }

  getDatePrecisionToTimeUnit(): string[] {
    return FP_DateTime._datePrecisionToTimeUnit;
  }
  ...
  static isoDateTime(date: Date, precision?: number): string {
    ...
    if (precision > 0) {
      rtn += '-' + formatNum(date.getMonth() + 1, 2);
      if (precision > 1) {
        rtn += '-' + formatNum(date.getDate(), 2);
        if (precision > 2) {
    ...
    if (precision > 2) {
      ...
      rtn += tzSign + formatNum(tzHour, 2) + ':' + formatNum(tzMin, 2);
    }
    return rtn;
  }
  ...
  static isoTime(date: Date, precision?: number): string {
    ...
    let rtn = '' + formatNum(date.getHours(), 2);
    if (precision > 0) {
      rtn += ':' + formatNum(date.getMinutes(), 2);
      if (precision > 1) {
        rtn += ':' + formatNum(date.getSeconds(), 2);
...
class FP_Time extends FP_TimeBase {
  ...
  _dateAtPrecision(precision: any) {
    var timeParts = this._getTimeParts();
    var md = this._getMatchData() ?? [];
    var timezoneOffset = md.length > 4 ? md[4] : '';
    // Get the date object first at the current precision.
    var thisPrecision = this._getPrecision() ?? 0;
  ...
  /**
     ...
     */
  _getMatchData() {
    return super._getMatchDataWithPrecision(timeRE, 2);
  }
  ...
  _getTimeParts() {
    if (!this.timeParts) {
      this.timeParts = super._getTimePartsBase(this._getMatchData());
    }
    return this.timeParts;
  }
  ...
  static checkString(str: string): FP_Time | null {
    let d: FP_Time | null = new FP_Time(str);
    if (!d._getMatchData())
      d = null;
    return d;
  }
   ...
   *  The inverse of _timeUnitToDatePrecision.
   */
  static _datePrecisionToTimeUnit = ["hour", "minute", "second", "millisecond"];

  // Implement abstract methods from FP_TimeBase
  getTimeUnitToDatePrecision(): Record<string, number> {
    return FP_Time._timeUnitToDatePrecision;
  }

  getDatePrecisionToTimeUnit(): string[] {
    return FP_Time._datePrecisionToTimeUnit;
  }
}
 ...
 */
function formatNum(num: number, len?: number): number | string {
 ...
 * @date the date to format
 * @precision the precision at which to terminate string string.  (This is
 *  probably not what was intended to be here.  Perhaps some text got cut off.)
 */
...
class FP_Date extends FP_DateTime {
   ...
   */
  _getMatchData() {
    return super._getMatchDataWithPrecision(dateRE, 2);
  }
}
...
class FP_Instant extends FP_DateTime {
  ...
  /**
     ...
     */
  _getMatchData() {
    return super._getMatchDataWithPrecision(instantRE, 5);
  }
}
...
class ResourceNode {
  ...
  // Static method - will be defined after class declaration
  static makeResNode: (
    data: any,
    parentResNode: ResourceNode | null,
    path: string | null,
      ...
      if (this.fhirNodeDataType) {
        ...
        if (match) {
          ...
          typeInfo = new TypeInfo({
            ...
            refType: this.parentResNode && this.model != null && this.model.path2RefType[this.parentResNode.path + '.' + this.propName] || null
          });
        }
      }
  ...
  /**
     ...
     */
  getParentResource(): ResourceNode | null {
    let node: ResourceNode | null = this;
    do {
  ...
  convertData() {
    if (!this.convertedData) {
      var data = this.data;
      if (data != null) {
        const cls = this.path ? TypeInfo.typeToClassWithCheckString[this.path] : null;
        if (cls) {
  ...
  fullPropertyName(): string | null {
    if (!this.propName) {
      if (!this.parentResNode && this.path === this.fhirNodeDataType)
        return this.fhirNodeDataType;
      return null;
    }
 ...
 */
ResourceNode.makeResNode = function (
  data: any,
  parentResNode: ResourceNode | null,
  path: string | null,
  _data?: any,
...
class TypeInfo {
  ...
  static FhirReference: TypeInfo;
  static FhirCanonical: TypeInfo;

  // Static methods - will be defined after class declaration
  ...
  static isType: (type: any, superType: any, model?: any) => boolean;
  static typeToClassWithCheckString: Record<string, any>;

  constructor({ name, namespace, refType = null }: TypeInfoConstructorArgs) {
    this.name = name;
  ...
  /**
     ...
     * @return {boolean}
     */
  is(other: TypeInfo, model?: Model | null): boolean {
    if (
  ...
  /**
     ...
     */
  isConvertibleTo(other: TypeInfo, model: Model): boolean {
 ...
 * @return {boolean}
 */
TypeInfo.isType = function (type: any, superType: any, model?: any): boolean {
  do {
    if (type === superType) {
      return true;
    }
 ...
 * @return {TypeInfo}
 */
TypeInfo.createByValueInSystemNamespace = function (value: any): TypeInfo {
 ...
 * @return {TypeInfo}
 */
TypeInfo.fromValue = function (value: any): TypeInfo {
 ...
 * @return {boolean}
 */
TypeInfo.isPrimitive = function (typeInfo: TypeInfo): boolean {
 ...
 *  otherwise false.
 */
TypeInfo.isPrimitiveValue = function (value: any): boolean {
 ...
 * @return {Array<*>}
 */
function typeFn(coll: any[]): any[] {
  return coll.map((value: any) => {
 ...
 * @return {boolean|[]}
 */
function isFn(this: FHIRPathContext, coll: any[], typeInfo: TypeInfo): boolean | any[] {
  if (coll.length === 0) {
    return [];
  }
 ...
 * @return {Array<*>}
 */
function asFn(this: FHIRPathContext, coll: any[], typeInfo: TypeInfo): any[] {
  if (coll.length === 0) {
    return [];
  }
 ...
 */
function toJSON(obj: any, space?: string | number) {
  return JSON.stringify(obj, (_, i) => typeof i === 'bigint' ? i.toString() : i, space);
}

export {
  asFn, dateRE, dateTimeRE, FP_Date,
  FP_DateTime,
  FP_Instant,
  FP_Time, FP_TimeBase, instantRE, isFn, ResourceNode, timeRE, toJSON, typeFn, TypeInfo
};
